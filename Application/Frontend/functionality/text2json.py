...
from functionality.validators_detectors import required_phrase_validation, detect_personal_details,detect_prohibited_phrases
from pydantic import BaseModel, Field

class conversation_format(BaseModel):
    """
    Check format for the conversation_json generated
    """
    start_time: float = Field(..., ge=0, description="Dialog start time for the speaker")
    end_time: float = Field(..., ge=0, description="Dialog end time for the speaker")
    speaker: str = Field(..., description="Identify if it is client or handler speaking")
    text: str = Field(..., description="Speaker's dialog")
    sentiment: str = Field(..., description="Sentiment of the dialog (positive, neutral or negative)")
    req_phrase_cat: List[str] = Field(..., description="List of required phrases categories in dialog. Eg greeting or disclaimer")
    pil_category: List[str] = Field(..., description="List of detected personal identifiable information leaks")
    prohibited_words: List[str] = Field(..., description="List of prohibited words detected in the dialog")

class attrb_format(BaseModel):
    """
    Check format for the attr_params generated
    """
    total_agent_time: float = Field(..., ge=0, description="Total time the agent spoke")
    total_customer_time: float = Field(..., ge=0, description="Total time the customer spoke")
    total_agent_words: int = Field(..., ge=0, description="Total number of words spoken by the agent")
    total_customers_words: int = Field(..., ge=0, description="Total number of words spoken by the customer")
    total_greetings: int = Field(..., ge=0, description="Number of greetings detected in the transcript")
    total_disclaimers: int = Field(..., ge=0, description="Total number of disclaimers detected in the transcript")
    total_closures: int = Field(..., ge=0, description="Total number of  closures detected in the transcript")
    total_pil: int = Field(..., ge=0, description="Total number personal identifiable information (PII) mentions")
    total_prohibited_words: int = Field(..., ge=0, description="Total number of prohibited words used")

class sentiment_format(BaseModel):
    """
    Check format for the sentiment_counts generated
    """
    speakers: Dict[str, Dict[str, int]] = Field(..., description="Count of positive, neutral amd negative sentiments at an overall level and for each speaker")


def text_to_json(text) -> Tuple[List[conversation_format], attrb_format, sentiment_format]:
    """
    Convert the text transcript into a json object. 
    ...
    sentiment_counts = sentiment_counts =  {"speakers": {
